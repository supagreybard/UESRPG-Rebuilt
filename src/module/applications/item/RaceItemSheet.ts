...
export class RaceItemSheet extends BaseRuleItemSheet {
  ...
  async #prepareGrantField(
    ...
    if (!(sourceItem instanceof Item)) {
      return {
        index,
        type: storedType,
        typeLabel: this.#getGrantTypeLabel(storedType),
        slug: storedSlug,
        sourceUuid,
        sourceName,
        optionGroup,
    ...
    return {
      ...
      typeLabel: this.#getGrantTypeLabel(String(sourceItem.type)),
      ...
      return [
        {
          ...
          typeLabel: this.#getGrantParameterTypeLabel(parameter.type),
  ...
  #getGrantTypeLabel(value: string): string {
    if (value === GRANT_TYPES.trait || value === GRANT_TYPES.power) {
      return localize(LABELS.itemTypes[value]);
    }

    switch (value) {
      case GRANT_TYPES.skill:
        return localize('UESRPG.GrantTypes.skill');
      case GRANT_TYPES.talent:
        return localize('UESRPG.GrantTypes.talent');
      case GRANT_TYPES.equipment_training:
        return localize('UESRPG.GrantTypes.equipmentTraining');
    }

    return value;
  }

  #getGrantParameterTypeLabel(value: unknown): string {
    switch (this.#readGrantParameterType(value)) {
      case 'number':
        return localize('UESRPG.ParameterTypes.number');
      case 'roll_formula':
        return localize('UESRPG.ParameterTypes.rollFormula');
      case 'text':
      default:
        return localize('UESRPG.ParameterTypes.text');
    }
  }
